...
export async function GET(request: Request) {
  try {
    ...
    // Get discount report using database function
    const { data: discounts, error } = await supabase.rpc('get_discount_report', {
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to generate discount report' },
      { status: 500 }
    )
  }
}
