...
function PayPalButtonInner({ orderId, amount, currency, items, shippingCost, discount = 0, tax = 0, onSuccess, onError }: PayPalCheckoutProps) {
  ...
  if (isRejected) {
    console.error('[PAYPAL-SDK] Script rejected - check NEXT_PUBLIC_PAYPAL_CLIENT_ID or currency:', currency)
    return (
      <div className="flex flex-col items-center justify-center py-8 gap-3">
...
export function PayPalCheckout(props: PayPalCheckoutProps) {
  ...
  if (!clientId || clientId === 'test') {
    console.error('[PAYPAL-SDK] NEXT_PUBLIC_PAYPAL_CLIENT_ID is not set')
  }
